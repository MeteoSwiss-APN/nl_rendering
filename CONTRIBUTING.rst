...
Report bugs at https://github.com/MeteoSwiss-APN/nl_rendering/issues.
...
Namelist Rendering could always use more documentation, whether as part of the official Namelist Rendering docs, in docstrings, or even on the web in blog posts, articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/MeteoSwiss-APN/nl_rendering/issues.
...
Ready to contribute? Here's how to set up `nl_rendering` for local development.

1. Fork the `nl_rendering` repo on GitHub.
2. Clone your fork locally::

        $ git clone git@github.com:your_name_here/nl_rendering.git
...
3. Create a virtual environment and install the development dependencies::
    ...
    This will create a virtual environment named ``nl_rendering`` (change with ``VENV_NAME=my_venv``) in ``./venv`` (change with ``VEND_DIR=path/to/venv``) and install the following dependencies:
...
To run a subset of tests::
    ...
    $ pytest tests.test_nl_rendering
...
.. list-table:: Structure
   ...
   * -  src/
     -  Source folder, with the main package in ``src/nl_rendering``.
   * -  tests/
   ...
   * -  USAGE.txt
     -  Instruction on using nl_rendering
   * -  VERSION
...
By default, a single executable script called nl_rendering is provided.
It is created when the package is installed.
When you call it, the main function (``cli``) in ``src/nl_rendering/cli.py`` is called.
...
When the package is installed, a executable script is created in the Python's bin folder with the name ``nl_rendering``.
If a user calls this script, the function ``main`` in the file ``src/nl_rendering/cli.py`` is called.
